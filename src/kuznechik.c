/**
 * @file
 * @brief Реализация алгоритма "Кузнечик"
 */

#include <string.h>

#include "kuznechik.h"
#include "table.h"

/** @brief Нелинейное биективное преобразование множества двоичных векторов. */
static const unsigned char kPi[256] =
{
	252, 238, 221,  17, 207, 110,  49,  22, 251, 196, 250, 218,  35, 197,   4,  77, 
	233, 119, 240, 219, 147,  46, 153, 186,  23,  54, 241, 187,  20, 205,  95, 193,
	249,  24, 101,  90, 226,  92, 239,  33, 129,  28,  60,  66,	139,   1, 142,  79,
	  5, 132,   2, 174, 227, 106, 143, 160,   6,  11, 237, 152, 127, 212, 211,  31,
	235,  52,  44,  81,	234, 200,  72, 171, 242,  42, 104, 162, 253,  58, 206, 204,
	181, 112,  14,  86,   8,  12, 118,  18, 191, 114,  19,  71, 156, 183,  93, 135,
	 21, 161, 150,  41,  16, 123, 154, 199, 243, 145, 120, 111, 157, 158, 178, 177,
	 50, 117,  25,  61, 255,  53, 138, 126, 109,  84, 198, 128, 195, 189,  13,  87,
	223, 245,  36, 169,  62, 168,  67, 201, 215, 121, 214, 246, 124,  34, 185,   3,
	224,  15, 236, 222, 122, 148, 176, 188, 220, 232,  40,  80,  78,  51,  10,  74,
	167, 151,  96, 115,  30,   0,  98,  68,  26, 184,  56, 130, 100, 159,  38,  65,
	173,  69,  70, 146,  39,  94,  85,  47, 140, 163, 165, 125, 105, 213, 149,  59,
	  7,  88, 179,  64, 134, 172,  29, 247,  48,  55, 107, 228,	136, 217, 231, 137,
	225,  27, 131,  73,  76,  63, 248, 254, 141,  83, 170, 144, 202, 216, 133,  97,
	 32, 113, 103, 164,  45,  43,   9,  91, 203, 155,  37, 208, 190, 229, 108,  82,
	 89, 166, 116, 210, 230, 244, 180, 192,	209, 102, 175, 194,  57,  75,  99, 182
};

/** @brief Обратное нелинейное биективное преобразование множества двоичных векторов. */
static const unsigned char kReversePi[256] =
{
     
};

/** @brief Коэффициенты умножения в преобразовании l */
static const  unsigned char kB[16] = {148, 32, 133, 16, 194, 192, 1, 251, 1, 192, 194, 16, 133, 32, 148, 1};

static int funcX(unsigned char* a, unsigned char* b, unsigned char* outdata)
{
     unsigned int i;

     if (!a || !b || !outdata)
     {
          return -1;
     }

     for(i = 0; i < 16; ++i)
     {
          outdata[i] = a[i] ^ b[i];
     }

     return 0;
}

static int funcS(unsigned char* indata, unsigned char* outdata)
{
     size_t i = 0;

     if(!indata || !outdata)
     {
          return -1;
     }

     for(i = 0; i < 16; ++i)
     {
          outdata[i] = kPi[indata[i]];
     }

     return 0;
}

static int funcReverseS(unsigned char* indata, unsigned char*  outdata)
{
     unsigned int i;

     if(!indata || !outdata)
     {
          return -1;
     }

     for(i = 0; i < 16; ++i)
          outdata[i] = kReversePi[indata[i]];

     return 0;
}

static int funcR(unsigned char* indata, unsigned char* outdata)
{
     size_t i;
     unsigned char sum = 0;

     if(!indata || !outdata)
     {
          return -1;
     }
     
     for(i = 0; i < 16; ++i)
     {
          sum ^= multTable[indata[i]*256 + kB[i]];
     }

     outdata[0] = sum;
     memcpy(outdata+1, indata, 15);

     return 0;
}

static int funcReverseR(unsigned char* indata, unsigned char* outdata)
{
     unsigned char tmp[16];
     unsigned char sum = 0;
     unsigned int i;

     if(!indata || !outdata)
     {
          return -1;
     }

     memcpy(tmp, indata+1, 15);
     tmp[15] = indata[0];

     
     for(i = 0; i < 16; ++i)
     {
          sum ^= multTable[tmp[i]*256 + kB[i]];
     }

     memcpy(outdata, tmp, 15); 
     outdata[15] = sum;
     
     return 0;
}

static int funcL(unsigned char* indata, unsigned char* outdata)
{
     unsigned char tmp[16];
     int i = 0;

     if(!indata || !outdata)
     {
          return -1;
     }

     memcpy(tmp, indata, 16);

     for(i = 0; i < 16; ++i)
     {
          funcR(tmp, outdata);
          memcpy(tmp, outdata, 16);
     }

     return 0;
}

static int funcReverseL(unsigned char* indata, unsigned char* outdata)
{
     unsigned char tmp[16];
     unsigned int i;

     if(!indata || !outdata)
     {
          return -1;
     }

     memcpy(tmp, indata, 16);

     for(i = 0; i < 16; ++i)
     {
          funcReverseR(tmp, outdata);
          memcpy(tmp, outdata, 16);
     }

     return 0;
}

static int funcLSX(unsigned char* a, unsigned char* b, unsigned char* outdata)
{
     unsigned char temp1[16];
     unsigned char temp2[16];

     if(!a || !b || !outdata)
     {
          return -1;
     }

     funcX(a, b, temp1);
     funcS(temp1, temp2);
     funcL(temp2, outdata);

     return 0;
}

static int funcReverseLSX(unsigned char* a, unsigned char* b, unsigned char* outdata)
{
     unsigned char temp1[16];
     unsigned char temp2[16];

     if(!a || !b || !outdata)
     {
          return -1;
     }

     funcX(a, b, temp1);
     funcReverseL(temp1, temp2);
     funcReverseS(temp2, outdata);

     return 0;
}

static int funcF(unsigned char* inputKey, unsigned char* inputKeySecond, unsigned char* iterationConst, unsigned char* outputKey, unsigned char* outputKeySecond)
{
     unsigned char temp1[16];
     unsigned char temp2[16];

     if(!inputKey || !inputKeySecond || !iterationConst || !outputKey || !outputKeySecond)
     {
          return -1;
     }

     funcLSX(inputKey, iterationConst, temp1);
     funcX(temp1, inputKeySecond, temp2);

     memcpy(outputKeySecond, inputKey, 16);
     memcpy(outputKey, temp2, 16);   

     return 0;
}

static int funcC(unsigned char number, unsigned char* output)
{
     unsigned char tempI[16];

     if(!output)
     {
          return -1;
     }
     
     memset( tempI, 0, 15 );
     tempI[15] = number;
     funcL(tempI, output);

     return 0;
}

int kuznechik_expkey(unsigned char* masterKey, unsigned char* keys)
{
     unsigned char C[16];
     unsigned char temp1[16];
     unsigned char temp2[16];
     unsigned char j, i;


     if(!masterKey || !keys)
     {
          return -1;
     }
     
     memcpy(keys, masterKey, 16);
     memcpy(keys + 16, masterKey + 16, 16);

     for(j = 0; j < 4; ++j)
     {
          memcpy(temp1, keys + j * 2 * 16, 16);
          memcpy(temp2, keys + (j * 2 + 1) * 16, 16);

          for( i = 1; i < 8; ++i )
          {
               funcC(j*8+i, C);
               funcF(temp1, temp2, C, temp1, temp2);
          }
          
          funcC(j*8+8, C);
          funcF(temp1, temp2, C, temp1, temp2);

          memcpy(keys + (j * 2 + 2) * 16, temp1, 16);
          memcpy(keys + (j * 2 + 3) * 16, temp2, 16);
     }
    
     return 0;
}

int kuznechik_encrypt(unsigned char* plainText, unsigned char* chipherText, unsigned char* keys)
{
     unsigned char xTemp[16];
     unsigned char yTemp[16];
     int i;

     if(!plainText || !chipherText || !keys)
     {
          return -1;
     }

     memcpy(xTemp, plainText, 16);
     
     for(i = 0; i < 9; ++i)
     {
          funcLSX(xTemp, keys + 16*i, yTemp);
          memcpy(xTemp, yTemp, 16);
     }
     funcX(yTemp, keys+9*16, chipherText);

     return 0;
}

int kuznechik_decrypt(unsigned char* chipherText, unsigned char* plainText, unsigned char* keys)
{
     unsigned char xTemp[16];
     unsigned char yTemp[16];
     int i;

     if(!plainText || !chipherText || !keys)
     {
          return -1;
     }

     memcpy(xTemp, chipherText, 16);
     for(i = 0; i < 9; ++i)
     {
          funcReverseLSX(xTemp, keys+(9-i)*16, yTemp);
          memcpy(xTemp, yTemp, 16);
     }
     funcX(yTemp, keys, plainText);

     return 0;
}
